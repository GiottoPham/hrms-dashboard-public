import { Button, IconButton, Avatar } from '@mui/material'
import type { Column } from 'react-table'
import { Table } from '@frontend/framework/Table'
import EditIcon from '@mui/icons-material/Edit'
import { UpDownIcon } from '@frontend/framework/icons/UpDownIcon'
import { useState } from 'react'
import ChevronRightIcon from '@mui/icons-material/ChevronRight'
import ChevronLeftIcon from '@mui/icons-material/ChevronLeft'
import type { Employee } from '@frontend/types/employee'

type CreateHeaderInput = {
  headerText: string
  sortBy?: keyof Employee
}

const createHeader = ({ headerText, sortBy }: CreateHeaderInput) => {
  const Header = () => {
    const [sort, setSort] = useState<{
      sortBy?: keyof Employee
      sortOrder: string
    }>({ sortBy: 'id', sortOrder: 'asc' })

    return (
      <header className="flex items-center">
        {headerText}
        {sortBy && (
          <IconButton
            classes={{ root: 'p-1 w-8 h-8' }}
            onClick={() => {
              if (sort.sortOrder === 'asc')
                setSort((prev) => ({
                  ...prev,
                  sortBy: sortBy,
                  sortOrder: 'desc',
                }))
              else setSort((prev) => ({ ...prev, sortOrder: 'asc' }))
            }}
          >
            {sort?.sortBy !== sortBy && <UpDownIcon />}
            {sort?.sortBy === sortBy && sort.sortOrder === 'asc' && (
              <UpDownIcon isUp={true} />
            )}
            {sort?.sortBy === sortBy && sort.sortOrder === 'desc' && (
              <UpDownIcon isUp={false} />
            )}
          </IconButton>
        )}
      </header>
    )
  }

  return Header
}
const columns: Column<Employee>[] = [
  {
    Header: createHeader({ headerText: 'ID', sortBy: 'id' }),
    accessor: 'id',
    Cell: ({ value }) => <p>{value}</p>,
    width: 'w-[68px]',
  },
  {
    Header: createHeader({ headerText: 'Avatar' }),
    accessor: 'avatar',
    Cell: ({ value, row }) => (
      <Avatar
        classes={{ root: 'h-8 w-8 text-lg bg-primary' }}
        alt={row.original.name}
        src={value}
      />
    ),
    width: 'w-[68px]',
  },
  {
    Header: createHeader({ headerText: 'Name', sortBy: 'name' }),
    accessor: 'name',
    Cell: ({ value }) => <p>{value}</p>,
    width: 'w-[250px]',
  },
  {
    Header: createHeader({ headerText: 'Job Title', sortBy: 'jobTitle' }),
    accessor: 'jobTitle',
    Cell: ({ value }) => <p>{value}</p>,
    width: 'w-[250px]',
  },
  {
    Header: createHeader({ headerText: 'Unit', sortBy: 'unit' }),
    accessor: 'unit',
    Cell: ({ value }) => <p>{value}</p>,
    width: 'w-[250px]',
  },
  {
    Header: createHeader({ headerText: 'Email', sortBy: 'email' }),
    accessor: 'email',
    Cell: ({ value }) => <p>{value}</p>,
    width: 'w-[250px]',
  },
  {
    Header: createHeader({ headerText: 'Contact', sortBy: 'contact' }),
    accessor: 'contact',
    Cell: ({ value }) => <p>{value}</p>,
    width: 'w-[150px]',
  },
  {
    id: 'actionCell',
    accessor: 'id',
    Cell: () => (
      <div className="flex justify-end">
        <IconButton>
          <EditIcon className="w-5 h-5" />
        </IconButton>
      </div>
    ),
    width: 'w-[50px]',
  },
]
const employeesFake: Employee[] = [
  {
    id: 1,
    avatar: 'https://www.w3schools.com/howto/img_avatar.png',
    name: 'Pham Khang Nguyen',
    jobTitle: 'Frontend Developer',
    unit: 'Website Develop Team',
    email: 'giotto2015.py@gmail.com',
    contact: '0854662633',
  },
  {
    id: 2,
    avatar: 'https://www.w3schools.com/howto/img_avatar.png',
    name: 'Pham Khang Nguyen',
    jobTitle: 'Frontend Developer',
    unit: 'Website Develop Team',
    email: 'giotto2015.py@gmail.com',
    contact: '0854662633',
  },
  {
    id: 3,
    avatar: 'https://www.w3schools.com/howto/img_avatar.png',
    name: 'Pham Khang Nguyen',
    jobTitle: 'Frontend Developer',
    unit: 'Website Develop Team',
    email: 'giotto2015.py@gmail.com',
    contact: '0854662633',
  },
  {
    id: 4,
    avatar: 'https://www.w3schools.com/howto/img_avatar.png',
    name: 'Pham Khang Nguyen',
    jobTitle: 'Frontend Developer',
    unit: 'Website Develop Team',
    email: 'giotto2015.py@gmail.com',
    contact: '0854662633',
  },
  {
    id: 5,
    avatar: 'https://www.w3schools.com/howto/img_avatar.png',
    name: 'Pham Khang Nguyen',
    jobTitle: 'Frontend Developer',
    unit: 'Website Develop Team',
    email: 'giotto2015.py@gmail.com',
    contact: '0854662633',
  },
  {
    id: 6,
    avatar: 'https://www.w3schools.com/howto/img_avatar.png',
    name: 'Pham Khang Nguyen',
    jobTitle: 'Frontend Developer',
    unit: 'Website Develop Team',
    email: 'giotto2015.py@gmail.com',
    contact: '0854662633',
  },
]
export const EmployeeTable = () => {
  return (
    <div className="rounded px-10 py-10 flex flex-col">
      <Table<Employee> data={employeesFake} columns={columns} rowCount={5} />
      <div className="self-end mt-5">
        <Button
          classes={{
            root: 'min-w-0 w-10 h-10 bg-white border border-primary',
          }}
          color="inherit"
          variant="outlined"
        >
          <ChevronLeftIcon className="w-7 h-7 text-primary" />
        </Button>
        <Button
          classes={{
            root: 'min-w-0 w-10 h-10 bg-white border border-primary ml-5',
          }}
          color="inherit"
          variant="outlined"
        >
          <ChevronRightIcon className="w-7 h-7 text-primary" />
        </Button>
      </div>
    </div>
  )
}
